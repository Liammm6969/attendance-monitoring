    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-slate-600 text-sm mt-1">Review intern daily accomplishment logs</p>
        </div>
      ...
      <div className="flex gap-2">
        {[{ val: "", label: "All" }, { val: "pending", label: "Pending" }, { val: "reviewed", label: "Reviewed" }].map(({ val, label }) => (
          <button key={val} onClick={() => setStatusFilter(val)} className={`px-3 py-2 text-xs font-medium rounded-xl transition-all ${statusFilter === val ? "bg-indigo-600 text-white" : "bg-white text-slate-600 border border-slate-200 hover:text-slate-900"}`}>{label}</button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-slate-200 rounded-2xl py-12 text-center"><ClipboardList size={28} className="text-slate-500 mx-auto mb-2" /><p className="text-slate-500 text-sm">No logs found</p></div>
      ) : (
        <div className="space-y-3">
          {logs.map((log) => (
            <div key={log._id} className="bg-white border border-slate-200 hover:border-indigo-200 rounded-2xl p-5 transition-all">
              <div className="flex items-start justify-between gap-3 mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-slate-900 text-sm font-medium">{log.userId?.firstName} {log.userId?.lastName}</p>
              ...
              <p className="text-slate-700 text-sm whitespace-pre-wrap leading-relaxed">{log.tasks}</p>
              {log.adminNote && (
                <div className="mt-3 pt-3 border-t border-slate-200">
                  <p className="text-slate-500 text-xs mb-1">Your feedback:</p>
                  <p className="text-slate-700 text-sm italic">{log.adminNote}</p>
                </div>
              )}
              {log.status === "pending" && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-md shadow-2xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-slate-900 font-semibold">Review Log</h2>
              <button onClick={() => setReviewModal(null)} className="text-slate-500 hover:text-slate-900"><X size={18}/></button>
            </div>
            ...
            <textarea value={adminNote} onChange={(e) => setAdminNote(e.target.value)} placeholder="Add feedback or notes for the intern..." rows={3}
              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 text-sm placeholder-slate-500 focus:outline-none focus:border-indigo-500/60 focus:ring-1 focus:ring-indigo-500/30 resize-none transition-colors" />
            <div className="flex gap-3">
              <button onClick={() => setReviewModal(null)} className="flex-1 py-2.5 text-sm text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition-all">Cancel</button>
