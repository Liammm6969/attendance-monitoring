...
export const Layout = ({ children }: { children: React.ReactNode }) => {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-6 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-bold text-white text-sm leading-tight">OJT Tracker</p>
  ...
  return (
    <div className="flex h-screen bg-[#060b18] overflow-hidden">
      ...
      {/* Sidebar — desktop */}
      <aside className="hidden lg:flex w-64 flex-col bg-[#0a1628] border-r border-white/5 flex-shrink-0">
        <SidebarContent />
      </aside>

      {/* Sidebar — mobile drawer */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 flex flex-col bg-[#0a1628] border-r border-white/5 transform transition-transform duration-300 ease-in-out lg:hidden ${
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Topbar */}
        <header className="h-16 flex items-center gap-4 px-4 lg:px-6 border-b border-white/5 bg-[#0a1628]/50 backdrop-blur-sm flex-shrink-0">
          <button
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="hidden sm:block text-sm text-slate-300 font-medium">{user?.firstName} {user?.lastName}</span>
          </div>
        </header>
